...
void bump_init(bump_allocator* allocator, size_t size) {
...
void* bump_alloc(bump_allocator* allocator, size_t size) {
    if (allocator->offset + size > allocator->size) {
        fprintf(stderr, "Out of memory in bump allocator\n");
        return NULL;
    }
...
void* benchmark_thread(void *args) {
    ...
    bump_init(&allocator, t_args->upper * t_args->iterations * t_args->repeats);
